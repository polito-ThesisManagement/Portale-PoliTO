...
const breadcrumbConfig = {
    ...
    'proposte_di_tesi': { icon: <HiLightBulb size={17} />, label: 'carriera.proposte_di_tesi.title_half_lowercase', path: '/carriera/proposte_di_tesi' }
};
...
export default function MyBreadcrumb() {
    ...
    const renderBreadcrumbElement = (value, index) => {
        ...
        if (config) {
            return (
                <React.Fragment key={index}>
                    <div className="mybreadcrumb-element" onClick={() => navigate(config.path)}>
                        {config.icon}
                        {t(config.label)}
                    </div>
        ...
        if (index > 0 && pathnames[index - 1] === "proposte_di_tesi" && !isNaN(value)) {
            return (
                <React.Fragment key={index}>
                    <div className="mybreadcrumb-element" onClick={() => navigate(`/carriera/proposte_di_tesi/${value}`)}>
                        <FaFileLines size={14} />
        ...
        return (
            <React.Fragment key={index}>
                <div className="mybreadcrumb-element">
                    {value}
                </div>
                {!isLast && <div className="mybreadcrumb-separator"> / </div>}
            </React.Fragment>
