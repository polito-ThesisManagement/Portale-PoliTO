              ...
              <tr key={index}>
                ...
                {giorniSettimana.map((giorno, indice) => {
                  const filteredCourses = myFilteredCourses.filter(corso => {
                    return corso.orarioSettimanale.some(
                      obj => obj.giorno === giorniSettimana[indice] && obj.orario.substring(0, 5).includes(orario),
                    );
                    ...
                    <td key={indice} style={{ border: '2px ridge' }}>
                      <Container style={{ display: 'flex', position: 'relative' }}>
                        {filteredCourses.map((corso, courseIndex) => (
                          <Col
                            md={2 /* si puù aumentare numero corso sovrapponibili */}
                            key={courseIndex}
                            className="col-6"
                          >
                            <div
